...
PROMPT="%{$fg[cyan]%}[%~]
%{$reset_color%}%(#.%{$fg_bold[red]%}.%{$fg_bold[green]%})%n%{$reset_color%}%{$fg_bold[grey]%}@%m%{$reset_color%}%(?.%{$fg_bold[green]%}.%{$bg[red]%}%{$fg_bold[white]%}) 
>%{$reset_color%} "
SPROMPT="        Заменить $fg_bold[grey]'$reset_color$fg_bold[red]%R$reset_color$fg_bold[grey]'$reset_color на
$fg_bold[grey]'$reset_color$fg_bold[green]%r$reset_color$fg_bold[grey]'$reset_color?
($fg_bold[grey]Y$reset_color$fg_bold[green]es$reset_color, $fg_bold[grey]N$reset_color$fg_bold[red]o$reset_color,
$fg_bold[grey]A$reset_color$fg_bold[red]bort$reset_color, $fg_bold[grey]E$reset_color$fg_bold[blue]dit$reset_color)
...
precmd() {
  [[ -t 1 ]] || return
 case $TERM in
 *xterm*|rxvt|xfce4-terminal|(dt|k|E|a)term*) print -Pn "\e]0;[%~] %n@%m\a" ;;
...
preexec() {
 [[ -t 1 ]] || return
 case $TERM in
 *xterm*|rxvt|xfce4-terminal|(dt|k|E|a)term*) print -Pn "\e]0;<$1> [%~] %n@%m\a" ;;
...
zstyle ':completion:*:*:*:users' ignored-patterns adm apache bin daemon games gdm halt ident junkbust lp mail mailnull named news
#nfsnobody nobody nscd ntp operator pcap postgres radvd rpc rpcuser rpm shutdown squid sshd sync uucp vcsa xfs

function dialogrun; { rm -rf $(dialog --separate-output --checklist file 100 100 100 $(for l in $(ls -A); do echo "$l" "$(test -d $l &&
echo "dir" || echo "file")" 0; done) --stdout); clear  }
zle -N dialogrun
